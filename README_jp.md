...
**QAM Mapper**は、GNU Radioフレームワーク向けのカスタム同期ブロックです。入力バイトデータ（`unsigned char`）を指定されたQAM（直交振幅変調）コンスタレーションの複素数点にマッピングします。QAM-16（4x4グリッド）、QAM-64（8x8グリッド）、QAM-128（8x16グリッド）、QAM-256（16x16グリッド）をサポートし、グレイコードマッピングの有効/無効を選択可能です。
...
- **設定可能なパラメータ**:
  - **QAM Size**: コンスタレーションサイズ（16、64、128、または256）。
  - **Enable Gray Mapping**: グレイコードマッピングを適用するかどうか。
- **特徴**:
  - QAM-16（4x4）、QAM-64（8x8）、QAM-128（8x16）、QAM-256（16x16）をサポート。
  - グレイコード変換/逆変換を効率的に処理。
  - 入力値の範囲チェックとエラーログ出力。
...
QAM Size:
  ...
  16: 4x4グリッド（16点）のQAMコンスタレーション。
  64: 8x8グリッド（64点）のQAMコンスタレーション。
  128: 8x16グリッド（128点）のQAMコンスタレーション。
  256: 16x16グリッド（256点）のQAMコンスタレーション。
...
## 7.1 **generate_constellation**
  ...
  引数:
    qam_size (int): QAMサイズ（16、64、128、256）。

    戻り値: np.ndarray（複素数配列）。

詳細:
  QAM-16: 4x4グリッド（各軸4点、座標は{-3,-1,1,3}）。
  QAM-64: 8x8グリッド（各軸8点、座標は{-7,-5,-3,-1,1,3,5,7}）。
  QAM-128: 8x16グリッド（I軸8点、Q軸16点）。
  QAM-256: 16x16グリッド（各軸16点、座標は{-15,-13,...,13,15}）。
  平均パワーを1に正規化。
...
class TestQAMMapper(unittest.TestCase):
    def test_generate_constellation(self):
        for qam_size in [16, 64, 128, 256]:
            ...
            self.assertEqual(len(points), qam_size)
